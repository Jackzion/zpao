...
import com.ziio.zpao.constant.UserConstant;
...
import com.ziio.zpao.mapper.UserMapper;
import com.ziio.zpao.utils.AlgorithmUtils;
...
import static com.ziio.zpao.constant.UserConstant.USER_LOGIN_STATE;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
     ...
     * 相似度匹配
     * @param num 頁數
     * @param loginUser 登錄的用戶
     * @return  用戶列表
     */
    @Override
    public List<User> matchUsers(long num, User loginUser) {
        // 從user表查 id and tags(select * 花费多时间)
        QueryWrapper<User> queryWrapper = new QueryWrapper<>();
        ...
        // 按编辑距离由小到大排序 steam流進行排序
        List<Pair<User, Long>> topUserPairList = list.stream()
                .sorted((a, b) -> (int) (a.getValue() - b.getValue()))
                .limit(num)
                .collect(Collectors.toList());
        // 原本顺序的 userId 列表 轉 user列表
